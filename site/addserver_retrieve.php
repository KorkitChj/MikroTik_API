            ...
            foreach ($ARRAY2 as $vv) {
                if ($vv['name'] == $ARRAY[$i]['name']) {
                    ...
                    $manage = '<div class="btn-group btn-group-toggle" data-toggle="buttons">
                                <button class="btn btn-success" type="button" onclick="enableServer(\'' . $ARRAY[$i]['.id'] . '\')"><span title="เปิด" class="glyphicon glyphicon-ok"></span></button>
                                <button class="btn btn-danger" type="button"  onclick="disableServer(\'' . $ARRAY[$i]['.id'] . '\')"><span title="ปิด" class="glyphicon glyphicon-remove"></span></button>
                                <button class="btn btn-warning" type="button" data-toggle="modal" data-target="#editServerModal" onclick="editServer(\'' . $ARRAY[$i]['.id'] . '\')"><span title="แก้ไข" class="glyphicon glyphicon-edit"></span></button>
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#removeServerModal" onclick="removeServer(\'' . $ARRAY[$i]['.id'] . '\')"><span class="glyphicon glyphicon-trash"></span></button></div>';
                                break;
                }
            }
            if ($aa == "") {
