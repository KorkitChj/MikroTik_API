...
foreach ($result as $row) {
    ...
    $image = '';
	if($row["image_site"] != ''){
		$image = '<img src="../img/sitelogo/'.$row["image_site"].'" class="img-thumbnail" width="50" height="35" />';
	}else{
		$image = '';
    }
    $checkbox = '
    ...
    if ($API->connect($row['ip_address'].":".$port,$row['username'],$row['password'])) {
        ...
        <button type="button" class="btn btn-info btn-sm" onclick="window.location.href=\'../process/site_admin_router/site_conn.php?id='.$row['location_id'].'&conn='.$connz.'\';"><span title="เข้าบริหารจัดการ" class="glyphicon glyphicon-new-window"></span></button>
